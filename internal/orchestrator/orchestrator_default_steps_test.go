...
func TestRun_DefaultSteps_RealWiringWithFakeCLIs(t *testing.T) {
	...
	t.Setenv("HARNEST_TEST_BASE_SHA", strings.Repeat("a", 40))
	t.Setenv("HARNEST_TEST_TARGET_SHA", strings.Repeat("b", 40))
	t.Setenv("HARNEST_TEST_MERGE_SHA", strings.Repeat("c", 40))
	t.Setenv("HARNEST_TEST_BEST_SHA", strings.Repeat("d", 40))
...
func TestRun_DefaultSteps_RealWiringWithFakeCLIs_PolicyOnlyAdoptIgnoresBestBranchPushFailure(t *testing.T) {
	...
	stateDir := t.TempDir()
	t.Setenv("HARNEST_GIT_STATE_DIR", stateDir)
	cfg.ClaudeCLIPath = filepath.Join(binDir, "claude")
...
func TestRun_DefaultSteps_RealWiringWithFakeCLIs_PolicyOnlyAdoptIgnoresPostPushSentinelScript(t *testing.T) {
	...
	stateDir := t.TempDir()
	t.Setenv("HARNEST_GIT_STATE_DIR", stateDir)
	t.Setenv("HARNEST_TEST_SENTINEL_PATH", sentinelPath)
	cfg.ClaudeCLIPath = filepath.Join(binDir, "claude")
