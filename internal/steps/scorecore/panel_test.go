...
func TestPanelResolver_Resolve(t *testing.T) {
	...
	t.Run("compliance disagreement triggers arbiter", func(t *testing.T) {
		...
		arbiterOut.Compliance[0].Verdict = contracts.ComplianceVerdictValidException
		...
		require.NoError(t, err)
		assert.Equal(t, contracts.VerdictPathArbitrated, result.VerdictPath)
		require.Len(t, result.RawCompliance, 3)
		require.Len(t, result.FinalCompliance, 1)
		assert.Equal(t, contracts.ComplianceVerdictValidException, result.FinalCompliance[0].Verdict)
	})
