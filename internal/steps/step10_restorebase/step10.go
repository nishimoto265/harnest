...
//
//  1. Fetches PR metadata (title/body/mergeCommit/linked issues) via `gh`.
...
func readPersistedBaseSHA(path string) (string, bool, error) {
	...
	if err := validation.Instance().Var(sha, "required,sha1_hex"); err != nil {
		return "", false, fmt.Errorf("step10: persisted base.sha is not a 40-hex sha: %q: %w", sha, err)
	}
	return sha, true, nil
}
...
func (r *Runner) deriveBaseSHA(ctx context.Context, repoRoot string, pr PRInfo) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("step10: resolve merge commit first parent %s^1: %w", pr.MergeCommitOID, err)
	}
