...
func TestSuccessDiffBytes_ExcludesOnlyPolicyOverlayArtifacts(t *testing.T) {
	...
	require.NoError(t, os.MkdirAll(filepath.Join(repoDir, ".harnest", "lessons"), 0o755))
	require.NoError(t, os.MkdirAll(filepath.Join(repoDir, "harnest", "rules"), 0o755))
	...
	require.NoError(t, os.WriteFile(filepath.Join(repoDir, ".harnest", "lessons", "local.md"), []byte("lesson\n"), 0o644))
	require.NoError(t, os.WriteFile(filepath.Join(repoDir, "harnest", "rules-registry.jsonl"), []byte("{}\n"), 0o644))
	require.NoError(t, os.WriteFile(filepath.Join(repoDir, "harnest", "rules", "r-local.md"), []byte("rule\n"), 0o644))
	require.NoError(t, os.WriteFile(filepath.Join(repoDir, "harnest", "app.go"), []byte("package autoimprove\n"), 0o644))
	...
	assert.Contains(t, text, "harnest/app.go")
	assert.NotContains(t, text, ".harnest")
	assert.NotContains(t, text, "harnest/rules")
}
