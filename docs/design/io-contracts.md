...
- **step70 append 時に registry size を check**(24h gate と独立、rev19 強化、Codex rev18 R3 対応:sunset 巨大化に備え 1500 で auto index 起動):
  - 1500 件超 → warning log + **`<runs_base>/rules-idempotency-index.jsonl` 自動生成有効化(以降の step70/sunset append で同 transaction に index 行を append)**。`processed.jsonl` に `warning { step: 70, kind: 'registry_size_high', count }` append(step70 emit、pr/run_id は該当 run 文脈があれば含める)
  - 1800 件超 → 上記 + index lookup を mandatory(tail scan は fallback のみ)
  - 2000 件超 → hard alert(`processed.jsonl` に `warning { step: 70, kind: 'registry_size_critical', count }` append + stderr 強制、operator は retention/compaction 検討)
...
- **step 70 は promotion.lock 保持中に限り `promoting / promoted / rollback / needs_manual_recovery / warning` を自己 append 可**(rev9、Codex rev8 R1/R3 対応)。順序の正確性は lock と staged intention の stage 遷移で保証。
...
**Non-terminal events (resume対象)**: `started / step_done / interrupted / promoting / warning`

- `promoting { pr, run_id, step: 70 }` — step70 の intention planning 完了時 append。resume 時は intention.json から stage を読んで判断。
- `warning { pr?, run_id?, step, kind, count?, detail?, detail_overflow_ref? }` — 運用 alert。
  - rev22 変更 (Codex rev21 H3 対応): `pr` / `run_id` を **optional** に。PR 文脈で emit する場合は両 field を含める
  - resume 対象は `pr` field が present な warning のみ(PR-scoped resume ループはそれだけ見る)。`pr` 不在の warning は global telemetry 扱い、resume queue には乗らない
  - `kind` enum(closed、rev33 確定): `registry_size_high` / `registry_size_critical` / `rescue_retry` の 3 種のみ
...
- orchestrator 起動時、`queue.go` は以下の順序で処理:
  1. `processed.jsonl` から non-terminal PR を列挙(`started / step_done / interrupted / promoting / warning`)
...
8. **`processed.jsonl` への append は orchestrator のみ**。例外: (a) step 10 の `started`、(b) step 70 が `promotion.lock` 保持中に限り `promoting / promoted / rollback / needs_manual_recovery / warning` を自己 append 可。他の全 step は typed result を orchestrator に返却する契約。
9. **既存テストを壊さない**。
