...
**worktree は runs/ の外**:
agent worktree は `<worktree_base>/<runId>-pass{1,2}-a{1..N}/`、pass base は
`<worktree_base>/<runId>-pass{1,2}-base/` に置く。コード上は
`pass1WorktreePath(ctx, agent)` / `pass2WorktreePath(ctx, agent)` /
`PassBaseWorktreePath(ctx, pass)` で取得。実際のメタデータは
**`task-package.json.worktrees[]` と `task-package.json.pass_bases[]` が正本**。step 70 の cleanup はそれを読む。
...
- rev8 以前の log が残っている場合、起動時に警告を出して無視(reader は schema error で reject しない、単に skip)するか、`harnest run --from-scratch --pr <n>` でリセット
...
| 30 | `<run>/30/done.marker` (rev8 導入) | marker あり → skip。無い場合は `CollapseByKey (agent, judge_role, dimension)` で reduce し未完了 role のみ再実行。通常は `judge_primary` 1 体の single verdict が揃った時点で marker atomic write |
...
scoring は 2 層:
1. **Raw layer** (`scores-A-raw.jsonl`, `compliance-A-raw.jsonl`): judge_role ごとの出力を append-only で記録。通常 pipeline は `primary` のみを書く。secondary / arbiter は互換用に schema と reducer が残る。reducer rule:
...
2. **Final layer** (`scores-A.jsonl`, `compliance-A.jsonl`): 最終 verdict のみ append-only。通常は primary の single verdict。`CollapseByKey(agent, dimension)` で最新勝ち(judge_role は final には持たない)
...
**legacy arbiter rerun worked example** (互換 panel を使う場合の upstream 再実行時の正しい流れ):
...
- 初回で `repo.policy_branch` が未作成の場合は repo-local の既存 harness/policy artifact か default bootstrap を seed として snapshot し、初回 adopt で `repo.policy_branch` を作成できる。作成後は `repo.policy_branch` から hydrate する
- `pass1/base` を作って current policy/checklist/guidance/hooks を overlay し、pass1 の 3 agent worktree を fan out する。`pass2/base` と pass2 agent worktree は metadata を記録し、実体作成は step50 まで遅延する。agent 実装 commit は評価用の一時出力であり、永続 branch へ昇格しない
...
**出力**:
- `<run>/task-package.json` (`TaskPackageSchema` 準拠、`worktrees` に 6 WorktreeAllocation、`pass_bases` に 2 PassBaseAllocation を含む)
- `<run>/base.sha`
...
**処理**:
- configured implementer CLI を 3体並列起動
- 各 agent は実装 + checklist記入 + commit
- step20 は `pass1/base` 由来の worktree を使う。step50 は `pass2/base` に current policy + experiment lesson を overlay し、準備済み head を task-package に永続化してから pass2 agent worktree を作る
- 実行直前にも current policy snapshot を `.harnest/lessons/` と `.harnest/checklist.md` として worktree に overlay する。pass2 では experiment lesson も同じ overlay に追加する
...
**処理**:
- Absolute scoring: `judge_primary` 1 体で判定する
...
| `needs_manual_recovery` | 自動 rollback 不能。sentinel と一緒に intention を保持して operator 介入待ち | `finalized`(recover CLI による cleanup 完了後) のみ | `planning` の必須項目 + `recovery_reason` + `failed_step` 必須。`registry_append_result` は failure point に応じて optional | 自動進行しない。alert log + exit。`harnest recover` のみが進める |
...
|---|---|---|---|---|
| present | absent | `<runs_base>/needs-recovery/<run_id>.json` or `.aborted.json` 存在 | remote divergence / lease failure / manual abort | **自動進行せず alert log + exit**。operator は `harnest recover --run <id> [--rollback \| --adopt-anyway \| --inspect \| --mark-manual-abort \| --finalize-cleanup \| --clear-sentinel]` で解決 |
...
- **全 step block**(rev19、Codex rev18 R2 対応): sentinel 存在中は step10/20/30/40/50/60/70 と sunset の **全て** を pre-flight gate で reject。**唯一の例外は `harnest recover --run <id>`** のみ(operator 介入専用)
...
- sentinel 解除は `harnest recover` CLI のみが行う
...
| `worktrees_created` | `int` | no | `0..4` in pass-base fanout mode (`pass1/base` + 3 pass1 agents); legacy/direct mode can report up to `6` | `stepio.Step10Response.WorktreesCreated` |
...
`contracts.PassBaseAllocation` (`task-package.json.pass_bases[]`)

| field | type | required | constraints / notes | Go xref |
|---|---|---|---|---|
| `pass` | `int` | yes | `1 | 2`; `pass_bases` が存在する場合は pass1/pass2 各1件が必須 | `contracts.PassBaseAllocation.Pass` |
| `path` | `string` | yes | non-empty clean absolute path (`filepath.Clean(path)==path`; `.` / `..` segment 禁止) | `contracts.PassBaseAllocation.Path` |
| `branch` | `string` | yes | non-empty; worktree branch と全体一意 | `contracts.PassBaseAllocation.Branch` |
| `base_sha` | `string` | yes | `sha1_hex`; pass base を最初に切った commit | `contracts.PassBaseAllocation.BaseSHA` |
| `head_sha` | `string` | yes | `sha1_hex`; policy overlay 後の prepared head。pass2 は step50 で永続更新される | `contracts.PassBaseAllocation.HeadSHA` |

`contracts.TaskPackage` (`<run>/task-package.json`)

| field | type | required | constraints / notes | Go xref |
|---|---|---|---|---|
| `schema_version` | `string` | yes | `"1"`; `pass_bases` は v1 の optional additive field。strict reader は現行 binary の struct で読む | `contracts.TaskPackage.SchemaVersion` |
...
| `pass_bases` | `[]PassBaseAllocation` | no | absent は legacy/direct-from-base run。存在する場合は pass1/pass2 各1件必須で、path/branch は worktrees と全体一意 | `contracts.TaskPackage.PassBases` |
...
`harnest recover --run <id>` は `promotion.lock` 取得 → state 再読込 → 下記 matrix の cell に該当する動作のみ許可。それ以外は refuse + stderr に matrix の expected state を print。
...
- `harnest recover --run <id> --inspect`: promotion.lock 取得後 read-only で全 state(intention / decision / sentinel / remote HEAD / registry head)を print、終了時 lock release、副作用無し
- `harnest recover --run <id> --mark-manual-abort`: decision.json を `action: 'rollback', rollback_reason: 'manual_abort_pending_cleanup'` で書込 + sentinel を **`<runs_base>/needs-recovery/<run_id>.aborted.json` に rename**(block 継続) + processed `needs_manual_recovery { pr, run_id, step: '70', reason: 'manual_abort_pending_cleanup', failed_step: '70', at }` append。branch/registry 修復は operator 手動で、**sentinel は削除しない**(未修復 state で次 promotion が走るのを防止)。
- `harnest recover --run <id> --finalize-cleanup --remote-head <sha> --registry-head <sha> [--policy-head <sha>]`: operator が branch/registry/policy を手動で整合させた後のみ使う。**remote / registry SHA は必須**(rev11、Codex rev10 R2 対応)。`repo.policy_branch` 設定時は **policy SHA も必須**。promotion.lock 下で remote HEAD / registry head / policy_branch HEAD を再確認、全て一致で `.aborted.json` 削除、pipeline 復旧。片方でも不一致なら refuse
...
- 手動で失敗 PR を再試行したい場合は `harnest run --pr <n> --from-scratch` で新 run_id を払い出して再実行
