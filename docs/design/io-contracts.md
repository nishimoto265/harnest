...
startup 時の基準は `intention.json.stage`。contract として許可される persisted stage は `internal/contracts/intention.go` の enum と一致し、`planning` / `branch_pushed` / `registry_appended` / `decision_written` / `rolling_back_branch_reverted` / `rolling_back_registry_appended` / `rolling_back_decision_written` / `needs_manual_recovery` の 8 個に限る。加えて、`finalized` は intention 削除後の終端 pseudo-stage として扱う。
...
|---|---|---|---|---|
| `planning` | planning snapshot を atomic write した直後。branch push / registry append / decision write は未確定 | `branch_pushed`; `needs_manual_recovery`; intention 削除後の fresh planning | `idempotency_key`, `run_id`, `best_sha_before`, `target_sha`, `candidates_hash`, `started_at`。`registry_append_result` / `recovery_reason` / `failed_step` は不要 | 下記 planning decision tree を実行 |
...
| `registry_appended` | registry append 完了後、`registry_append_result` を保持している | `decision_written`; `rolling_back_branch_reverted`; `needs_manual_recovery` | `branch_pushed` の必須項目 + `registry_append_result` 必須 | decision write → promoted append → intention delete を再実行 |
| `decision_written` | `decision.json`(adopt) 書込み済み、intention 削除前 | `finalized` | `registry_appended` と同じ(`registry_append_result` 必須) | intention 削除 + `processed.jsonl` の `promoted` append を exactly-once で完了 |
...
| `registry_path` | `string` | yes | non-empty clean absolute path (`filepath.Clean(path)==path`; `.` / `..` segment 禁止) | `stepio.Step40Request.RegistryPath` |
...
| `registry_path` | `string` | yes | non-empty clean absolute path (`filepath.Clean(path)==path`; `.` / `..` segment 禁止) | `stepio.Step70Request.RegistryPath` |
...
| `diff_path` | `string` | yes | run 相対 path | `contracts.ManifestSuccess.DiffPath` |
| `session_path` | `string` | yes | run 相対 path | `contracts.ManifestSuccess.SessionPath` |
| `checklist_path` | `string` | yes | run 相対 path | `contracts.ManifestSuccess.ChecklistPath` |
...
| `rationale` | `string` | no | `max=500`; `exception` 時に人間向け説明 | `contracts.ChecklistItem.Rationale` |
...
| `proposed_body_path` | `string` | yes | non-empty relative path | `contracts.Candidate.ProposedBodyPath` |
...
| `agent_a` | `AgentID` | yes | `agent_id_fmt` | `contracts.PairwiseEntry.AgentA` |
| `agent_b` | `AgentID` | yes | `agent_id_fmt` | `contracts.PairwiseEntry.AgentB` |
