...
| `registry_path` | `string` | yes | non-empty absolute path を想定 | `stepio.Step40Request.RegistryPath` |
...
| `registry_path` | `string` | yes | non-empty absolute path を想定 | `stepio.Step70Request.RegistryPath` |
...
`Step70Response` の decode 2段階:
- plain `json.Unmarshal` は strict JSON + response-local invariant のみ通す。`DecodedAndBound() == false`
- `DecodeAndValidateStep70Response(data, req)` は request-bound invariant まで通した値だけ返し、`DecodedAndBound() == true`
...
| `path` | `string` | yes | non-empty absolute path | `contracts.WorktreeAllocation.Path` |
