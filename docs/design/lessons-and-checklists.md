...
repo-specific な learned policy は対象 repo の `.auto-improve/` に置く。
...
`checklist.md` は参照用であり、作業結果は `.auto-improve/work/checklist-result.md` に
記録する。
...
`auto-improve lessons verify-checklist-result` を呼ぶ。
...
```bash
auto-improve lessons new no-temp-artifact-commit \
  --checklist-item "作業用ファイルや一時ファイルを実装差分に含めない" \
  --severity high \
  --confidence high \
  --category git-hygiene

auto-improve lessons generate-checklist
auto-improve lessons generate-checklist --check
auto-improve lessons prepare-checklist-result
auto-improve lessons verify-checklist-result
auto-improve lessons install-guidance --provider claude,codex
```

`lessons new` は `.auto-improve/lessons/<id>.md` の skeleton を作る。
`generate-checklist` は active lessons から `.auto-improve/checklist.md` を生成する。
`--check` は CI / review 用で、既存 checklist が生成結果と一致しない場合に失敗する。
`prepare-checklist-result` は作業用 result を `.auto-improve/work/checklist-result.md` にコピーする。
...
`@.auto-improve/checklist.md` を参照させる。
...
- 通常利用では `.auto-improve/work/checklist-result.md` を作業用 result とする
