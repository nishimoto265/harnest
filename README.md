...
> ⚠️ **Status (2026-05-14)**: CLI / release packaging / Homebrew cask publish
> まで実行済みです。self-hosted Actions と実 macOS `launchctl` reload は
> 運用環境側の設定・認証に依存します。
...
target under `~/.harnest/repos/<owner>/<repo>`, stores per-repository
state under `~/.harnest/runs/<owner>__<repo>/`, and records registrations
in `~/.harnest/repositories.yaml`. Set `HARNEST_HOME` to relocate
...
- `harnest <repo-url>`
  Register/bootstrap a GitHub repository URL under `~/.harnest` and run
...
- `harnest clear <repo-url>`
  ...
  `~/.harnest/archives/cleared/<timestamp>/...` に退避し、次回実行で
  clean bootstrap できる状態にする。実リモートや policy branch は変更しない。
- `harnest clear --all`
  `HARNEST_HOME` 配下の generated state 全体を archive に退避する。
- `harnest sunset`
...
- `harnest lessons new <id> --checklist-item <text>`
  `.harnest/lessons/<id>.md` の lesson skeleton を作成する。
- `harnest lessons generate-checklist [--check]`
  active lessons から `.harnest/checklist.md` を生成、または stale か確認する。
- `harnest lessons prepare-checklist-result [--force]`
  `.harnest/checklist.md` を `.harnest/work/checklist-result.md` にコピーする。
...
`com.nishimoto265.harnest.<instance>`. Set
`HARNEST_INSTANCE=owner/repo` when installing or uninstalling multiple
...
`harnest`. That runner must already have `git`, `gh`, `jq`, `yq`, `lsof`,
...
- `launchctl start com.nishimoto265.harnest.<instance>` で launchd の次 tick を前倒し
